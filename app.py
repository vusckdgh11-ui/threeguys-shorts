...
import cv2
import numpy as np
from PySide6.QtCore import Qt, QThread, Signal, QTimer, QSettings, QPointF
...
from PySide6.QtGui import QImage, QPixmap, QFont, QColor, QPen, QBrush, QPainter
...
def choose_timeline(paths, target):
    ...
    for _, c in picked:
        if acc >= target:
            break
        left = target - acc
        if c.source_duration > left and left >= 1.2:
            c.end = c.start + left
        result.append(c)
...
def synthesize_typecast(text, out_wav, api_key, voice_id, tempo=1.0):
    ...
    if r.status_code != 200:
        raise RuntimeError(f"Typecast TTS 오류 ({r.status_code}): {r.text[:600]}")
...
def synthesize_google(text, out_wav, slow=False):
    ...
    gTTS(text=text, lang='ko', slow=bool(slow)).save(tmp_mp3)
...
def render_video(segs: List[Segment], out_mp4, provider, voice_data, api_key, voice_rate,
    ...
    try:
        if progress: progress(5, "장면별 TTS 생성 중...")
        audio_files = []
        for i, s in enumerate(segs):
            wav = tmp / f"voice_{i:02d}.wav"
            synthesize_line(provider, s.line, wav, voice_data, api_key, voice_rate)
            vd = max(0.2, wav_duration(wav))
            s.voice_duration = vd
            # one scene == one narration line; the visual cut is resized to that narration duration
            s.play_duration = max(1.4, vd + 0.18)
            audio_files.append(wav)
            if progress: progress(5 + int(30 * (i + 1) / max(1, len(segs))), f"TTS {i+1}/{len(segs)}")

        total = sum(s.duration for s in segs)
        ass = tmp / "captions.ass"
        build_ass(ass, segs, total, banner, caption_style, cap_x, cap_y, cap_size)

        ff = ffmpeg_exe()
        cmd = [ff, '-y']
        video_input_indices, audio_input_indices = [], []
        for s in segs:
            src_total = get_video_duration(s.path)
            available = max(0.2, src_total - s.start)
            read_dur = min(s.duration, available)
            video_input_indices.append(len(video_input_indices) + len(audio_input_indices))
            cmd += ['-ss', f'{s.start:.3f}', '-t', f'{read_dur:.3f}', '-i', ffpath(s.path)]
        # indices above are simply 0..n-1 because audio inputs are added after videos
        video_input_indices = list(range(len(segs)))
        for wav in audio_files:
            audio_input_indices.append(len(segs) + len(audio_input_indices))
            cmd += ['-i', ffpath(wav)]

        logo_input_index = None
        if logo_path and os.path.exists(logo_path):
            logo_input_index = len(segs) + len(audio_files)
        ...
        for i, s in enumerate(segs):
            src_total = get_video_duration(s.path)
            available = max(0.2, src_total - s.start)
            pad = max(0.0, s.duration - available)
            chain = f"[{i}:v]scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,fps=30,setsar=1"
            if effects and i % 4 == 2:
                chain += ",scale=1166:2074,crop=1080:1920"
            if pad > 0.01:
                chain += f",tpad=stop_mode=clone:stop_duration={pad:.3f}"
            chain += f",trim=duration={s.duration:.3f},setpts=PTS-STARTPTS[v{i}]"
            filters.append(chain); vlabels.append(f"[v{i}]")

            ai = audio_input_indices[i]
            filters.append(f"[{ai}:a]aresample=48000,apad=pad_dur={s.duration:.3f},atrim=0:{s.duration:.3f},asetpts=PTS-STARTPTS[a{i}]")
            alabels.append(f"[a{i}]")

        filters.append("".join(vlabels) + f"concat=n={len(segs)}:v=1:a=0[vcat]")
        filters.append("".join(alabels) + f"concat=n={len(segs)}:v=0:a=1[acat]")
        ...
        if logo_input_index is not None:
            ...
            filters.append(f"[vsub][logo]overlay={int(logo_x)}:{int(logo_y)}:format=auto:eof_action=repeat[vout]")
            final_label = '[vout]'

        cmd += ['-filter_complex', ';'.join(filters), '-map', final_label, '-map', '[acat]',
                '-c:v', 'libx264', '-preset', 'medium', '-crf', '19', '-pix_fmt', 'yuv420p',
                '-c:a', 'aac', '-b:a', '192k', '-t', f'{total:.3f}', '-movflags', '+faststart', ffpath(out_mp4)]
        if progress: progress(70, "영상·대본·TTS 동기화 렌더링 중...")
        p = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding='utf-8', errors='ignore')
        if p.returncode != 0:
...
class ResizePixmapItem(QGraphicsPixmapItem):
    ...
    def _corner_at(self, p):
        r = self.boundingRect(); h = self.HANDLE
        tests = {
    ...
    def mousePressEvent(self, event):
        ...
        if c:
            ...
            self.old_scene_rect = self.mapRectToScene(self.boundingRect()).boundingRect()
            event.accept(); return
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.resize_corner in ('br','tr'):
            ...
            new_x = r.right() - new_w
        new_scale = max(0.02, min(20.0, new_w / self.orig_w))
        self.setScale(new_scale)
        new_h = self.boundingRect().height() * new_scale
...
class PreviewView(QGraphicsView):
    ...
    def emit_logo(self):
        ...
        rect = self.logo_item.mapRectToScene(self.logo_item.boundingRect()).boundingRect()
...
class Main(QMainWindow):
    def __init__(self):
        ...
        center=QWidget(); center.setMinimumWidth(430); center.setMaximumWidth(450); cl=QVBoxLayout(center); cl.setContentsMargins(8,8,8,8)
        ...
        self.preview=PreviewView(); self.preview.logo_changed.connect(self.logo_changed); self.preview.caption_changed.connect(self.caption_changed); gpl.addWidget(self.preview,0,Qt.AlignHCenter)
        ...
        right_scroll=QScrollArea(); right_scroll.setWidgetResizable(True); right_scroll.setWidget(right_wrap); right_scroll.setMinimumWidth(520); splitter.addWidget(right_scroll)
    ...
    def preview_voice(self):
        ...
        try:
            ...
            if not out.exists(): synthesize_line(provider,"쓰리가이즈 특수청소입니다.",out,data,key,self.rate.value())
    ...
    def sync_script_from_box(self):
        ...
        if len(lines)!=len(self.segs): return False
    ...
    def do_render(self):
        ...
        args=(self.segs,out,provider,data,key,self.rate.value(),self.logo,self.lw.value(),self.lx.value(),self.ly.value(),self.effects.isChecked(),self.banner.isChecked(),self.caption_style.currentText(),self.cap_x,self.cap_y,self.cap_size)
